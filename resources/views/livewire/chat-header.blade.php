<div>
    ...
    <div x-show="$wire.isProfileOpen" id="drawer-profile" class="fixed top-0 right-0 z-50 w-screen md:w-96 h-screen p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800 border-s border-slate-600 {{ $isProfileOpen ? 'transform-none' : '' }}" tabindex="-1" aria-labelledby="drawer-profile-label">
        @if($userModel->alias_name)
            <button type="button" wire:click='toggleContactHeader' id="drawer-profile-label" class="inline-flex items-center mb-4 text-base font-semibold text-gray-500 dark:text-gray-400">
                <svg class="w-6 h-6 me-2.5 "  xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        ...
        <div class="flex flex-col items-center pb-10">
            ...
            @if(!$userModel->alias_name)
                <button type="button" wire:click='toggleContactHeader' class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-100 font-medium rounded-lg text-sm px-3 py-2 text-center inline-flex items-center dark:focus:ring-green-100 mt-4">
                    <svg class="w-5 h-5 me-2"  xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        ...
        <ul class="w-full space-y-2 text-gray-500 list- dark:text-gray-400">
            <li class="flex items-center">
                <button @click="alert('Under development')" type="button" class="w-full flex items-center hover:bg-gray-700 p-4 ps-2.5 rounded-lg text-red-500">
                    <svg class="w-6 h-6 me-3 text-red-400"  xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="m6 6 12 12m3-6a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>
                    </svg>
                    Block {{ $userModel->alias_name ?? $userModel->name }}
                </button>
            </li>
            <li class="flex items-center">
                <button @click="alert('Under development')" type="button" class="w-full flex items-center hover:bg-gray-700 p-4 ps-2.5 rounded-lg text-red-500">
                    ...
                    Report {{ $userModel->alias_name ?? $userModel->name }}
                </button>
            </li>
            <li class="flex items-center">
                <button @click="alert('Under development')" type="button" class="w-full flex items-center hover:bg-gray-700 p-4 ps-2.5 rounded-lg text-red-500">
    ...
    <div x-show="$wire.isMediaOpen" id="drawer-media" class="fixed top-0 right-0 z-50 w-screen md:w-96 h-screen p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800 border-s border-slate-600 {{ $isMediaOpen ? 'transform-none' : '' }}" tabindex="-1" aria-labelledby="drawer-media-label">
                                        ...
                                        <div class="group relative my-2.5">
                                            <div class="absolute w-full h-full bg-gray-900/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg flex items-center justify-center">
                                                <a href="{{ $row->fileUrl() }}" download="{{ $row->file_name }}" data-tooltip-target="download-image" class="inline-flex items-center justify-center rounded-full h-10 w-10 bg-white/30 hover:bg-white/50 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50">
                                                    <svg class="w-5 h-5 text-white"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 18">
                                        ...
                                        <div class="break-all flex justify-between items-start my-2.5 bg-gray-50 {{ $row->sender_id==auth()->id() ? 'dark:bg-green-800' : 'dark:bg-gray-600' }} rounded-xl p-2 w-full">
                                            <div class="me-2">
                                                <span class="flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-white pb-2">
                                                    ...
                                                    @if ($ext === 'pdf')
                                                        <!-- PDF -->
                                                        <svg fill="none"  class="w-5 h-5 shrink-0" viewBox="0 0 20 21">
                                                    ...
                                                    @elseif (in_array($ext, ['doc', 'docx']))
                                                        <!-- Word -->
                                                        <svg class="w-6 h-6 text-gray-800 dark:text-blue-500"  xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    @elseif (in_array($ext, ['xls', 'xlsx', 'csv']))
                                                        <!-- Excel -->
                                                        <svg class="w-6 h-6 text-gray-800 dark:text-green-500"  xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <div>
                                                <a href="{{ $row->fileUrl() }}" download="{{ $row->file_name }}" class="inline-flex self-center items-center p-2 text-sm font-medium text-center text-gray-900 bg-gray-50 rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-600 dark:hover:bg-gray-500 dark:focus:ring-gray-600" type="button">
                                                    <svg class="w-4 h-4 text-gray-900 dark:text-white"  xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div id="drawer-contact" class="fixed top-0 right-0 z-50 w-screen md:w-96 h-screen p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800 border-s border-slate-600 {{ $isAddContactHeaderOpen ? 'transform-none' : '' }}" tabindex="-1" aria-labelledby="drawer-contact-label">
        ...
        <button type="button" wire:click='toggleContactHeader' data-drawer-hide="drawer-contact" aria-controls="drawer-contact" class="mb-2 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
            <svg class="w-3 h-3"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
            </svg>
            <span class="sr-only">Close menu</span>
        ...
        @if($isAddContactHeaderOpen)
            <livewire:chat-contact-form :key="'chat-contact-form-'.time()" :isNewRecord="false" :user="$userModel" />
        @endif
                        ...
                        <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-600">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button wire:click="closePreview" type="button" 
                                    ...
                                    <svg class="w-5 h-5 text-gray-800 dark:text-white"  xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
